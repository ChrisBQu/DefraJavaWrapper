...
2. Move the pair of shared objects generated in the "Building the Shared Objects for Android" step to `src/main/jniLibs/arm64-v8a' and `src/main/jniLibs/x86_64` respectively.
3. Inside the `src/main/c` directory, execute the existing `build.sh` script.
4. Inside the root of the project, run the command `./gradlew assembleRelease`
  
To document fully what is happening, in step 3, running `build.sh` invokes the Clang compiler twice, generating `nativewrapper.so` for each of the two architectures. In step 4, the .aar will be created at `build/outputs/aar/defradb-android-release.aar`. This is the archive that can be used in an Android Studio project.
